...
int main (int argc, char *argv[]) {
    ...
    tareador_start_task("Llamada a la 1a Función");
    ffts1_planes(p1d, in_fftw);	
    tareador_end_task("Llamada a la 1a Función");
    
    tareador_start_task("Llamada a la 2a Función");
    transpose_xy_planes(tmp_fftw, in_fftw);
    tareador_end_task("Llamada a la 2a Función");
    
    tareador_start_task("Llamada a la 3a Función");
    ffts1_planes(p1d, tmp_fftw);
    tareador_end_task("Llamada a la 3a Función");
    
    tareador_start_task("Llamada a la 4a Función");
    transpose_zx_planes(in_fftw, tmp_fftw);
	tareador_end_task("Llamada a la 4a Función");
	
    tareador_start_task("Llamada a la 5a Función");
    ffts1_planes(p1d, in_fftw);
    tareador_end_task("Llamada a la 5a Función");
    
    tareador_start_task("Llamada a la 6a Función");
    transpose_zx_planes(tmp_fftw, in_fftw);
    tareador_end_task("Llamada a la 6a Función");
    
    tareador_start_task("Llamada a la 7a Función");
    transpose_xy_planes(in_fftw, tmp_fftw);    
    tareador_end_task("Llamada a la 7a Función");
